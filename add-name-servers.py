...
# now create juniper device object from PyEZ Device class and open this device.
for juniper in myvars['juniper_devices']:
    ...
    try:
        # Now try to open connection to device
        output = device.open()
    ...
    # otherwise raise specific error. commit_check should return jnpr.junos.exception.RpcError class
    try:
        commit_check = device.cfg.commit_check()
    except Exception as err:
        print type(err)
        print err.rsp
        break()
